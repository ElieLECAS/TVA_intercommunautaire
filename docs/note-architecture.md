...
Une campagne réelle de 200 numéros structurellement "valides" selon le substitut a ensuite renvoyé 75 % d'`invalide` côté VIES (0 valide sur 6 des 10 pays malgré 14 à 23 tentatives chacun). Ce taux ne prouve pas que le substitut se trompe sur le format : VIES ne vérifie pas qu'un format, il dit si le numéro est **réellement enregistré** — et ce jeu de données étant synthétique, la plupart des numéros structurellement plausibles n'ont probablement jamais correspondu à une vraie immatriculation, indépendamment de la justesse de la formule. Le seul numéro dont on est certain qu'il est réel (DANONE) ressort valide, ce qui va dans ce sens sans le prouver formellement. Limite assumée et surveillable : un `INVALID` de VIES sur une ligne jugée valide par le substitut reste visible dans `vies_verifications.detail`, jamais avalé silencieusement.
